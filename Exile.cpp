...
void Exile::PatchEnhancedExileRAM() {
	...
	// =============================================================================
	// 128-OBJECT EXPANSION PORT (mirror of PatchExileRAM's relocation for enhanced)
	// =============================================================================
	// Standard relocates object stacks from $0860-$0976 (16-slot, packed) to
	// $9600-$A700 (128-slot, 256-byte-per-stack) + rewrites 21 specific code sites
	// + bumps loop bounds. For enhanced, code layout is totally different — we
	// scanned the disassembly and found 192 instructions referencing $0860-$0985
	// that need their operand rewritten to the new $9600+ base.

	// --- Copy 16-slot object stacks to new 128-slot base addresses ---
	CopyRAM(0x0860, 0x9600, 0x10);   // object_stack_type
	CopyRAM(0x0870, 0x9700, 0x10);   // object_stack_sprite
	CopyRAM(0x0880, 0x9800, 0x10);   // object_stack_x_low
	CopyRAM(0x0891, 0x9900, 0x10);   // object_stack_x
	CopyRAM(0x08A3, 0x9A00, 0x10);   // object_stack_y_low
	CopyRAM(0x08B4, 0x9B00, 0x10);   // object_stack_y
	CopyRAM(0x08C6, 0x9C00, 0x10);   // object_stack_flags
	CopyRAM(0x08D6, 0x9D00, 0x10);   // object_stack_palette
	CopyRAM(0x08E6, 0x9E00, 0x10);   // object_stack_velocity_x
	CopyRAM(0x08F6, 0x9F00, 0x10);   // object_stack_velocity_y
	CopyRAM(0x0906, 0xA000, 0x10);   // object_stack_target
	CopyRAM(0x0916, 0xA100, 0x10);   // object_stack_tx
	CopyRAM(0x0926, 0xA200, 0x10);   // object_stack_energy
	CopyRAM(0x0936, 0xA300, 0x10);   // object_stack_ty
	CopyRAM(0x0946, 0xA400, 0x10);   // object_stack_supporting
	...
	// --- Rewrite 192 instruction operands from $08xx to $96xx+ ---
	// Each pair is {site, new_base} where site is the instruction address (operand
	// lives at site+1 / site+2 for a 3-byte instr).
	// Main-RAM sites ($0100-$7FFF) — write directly to BBC.ram.
	static const uint16_t kMainObjSites[154][2] = {
		{0x0BAB,0x9E00},{0x0BB0,0x9F00},{0x0BB4,0x9E00},{0x0BB9,0x9F00},{0x0BCD,0x9600},
		{0x0C25,0x9900},{0x0C2B,0x9B00},{0x0C34,0xA200},{0x0C3F,0x9800},{0x0C45,0x9A00},
		{0x0CAF,0x9C00},{0x0CB8,0x9C00},{0x0CF0,0x9C00},{0x0CF5,0x9C00},{0x0CFA,0xA500},
		{0x1A33,0x9B00},{0x1A3F,0x9C00},{0x1A4B,0x9900},{0x1A52,0x9800},{0x1A59,0x9F00},
		{0x1A60,0x9E00},{0x1A67,0x9A00},{0x1A6E,0x9700},{0x1A75,0x9D00},{0x1A7C,0x9600},
		{0x1A81,0xA600},{0x1A86,0xA000},{0x1A91,0xA100},{0x1A96,0xA200},{0x1A9B,0xA300},
		{0x1AA0,0xA400},{0x1AA5,0xA700},{0x1AAA,0xA500},{0x1B21,0x9700},{0x1B30,0x9A00},
		{0x1B37,0x9B00},{0x1B4A,0x9C00},{0x1B58,0x9800},{0x1B60,0x9900},{0x1B6C,0x9C00},
		{0x1CF3,0x9600},{0x1DAD,0x9B00},{0x1DB2,0x9900},{0x1DB7,0x9800},{0x1DBC,0x9F00},
		{0x1DC1,0x9E00},{0x1DC6,0x9C00},{0x1DCB,0x9A00},{0x1DD0,0x9700},{0x1DD5,0x9D00},
		{0x1DDA,0x9600},{0x1DDF,0xA600},{0x1DE8,0xA000},{0x1DED,0xA100},{0x1DF2,0xA200},
		{0x1DF7,0xA300},{0x1DFE,0xA400},{0x1E03,0xA700},{0x1E08,0xA500},{0x1E55,0x9600},
		{0x1E5E,0xA400},{0x1E63,0xA400},{0x1E67,0xA000},{0x1E6F,0xA000},{0x1EA9,0x9B00},
		{0x1EB7,0x9C00},{0x1ED7,0xA600},{0x1EEF,0x9B00},{0x1EFE,0x9D00},{0x1F04,0x9700},
		{0x1F09,0x9C00},{0x1F0E,0xA400},{0x1F12,0xA000},{0x1F17,0xA600},{0x1F1A,0xA700},
		{0x1F1D,0xA500},{0x1F20,0x9E00},{0x1F23,0x9F00},{0x1F27,0x9600},{0x1FE6,0x9600},
		{0x22D6,0x9700},{0x22DD,0x9800},{0x22E2,0x9900},{0x22ED,0x9A00},{0x22F2,0x9B00},
		{0x2520,0x9C00},{0x2525,0x9C00},{0x2528,0xA200},{0x2537,0xA200},{0x2545,0x9C00},
		{0x254A,0x9C00},{0x25E2,0x9B00},{0x25E7,0x9900},{0x25EA,0x9800},{0x25EF,0x9A00},
		{0x26DF,0x9A00},{0x26E2,0x9900},{0x26E7,0x9E00},{0x26EA,0x9B00},{0x26EF,0x9F00},
		{0x26F4,0xA700},{0x2728,0x9B00},{0x2769,0x9B00},{0x276E,0xA000},{0x277C,0x9900},
		{0x2787,0x9B00},{0x27E6,0xA000},{0x27EE,0x9E00},{0x27F1,0x9E00},{0x2898,0x9900},
		{0x289B,0xA100},{0x28A0,0x9B00},{0x28A3,0xA300},{0x28A9,0x9900},{0x28AE,0x9B00},
		{0x28B2,0x9900},{0x28B7,0x9B00},{0x28C1,0x9900},{0x28C6,0x9B00},{0x28CB,0x9800},
		{0x28CE,0x9A00},{0x2DD8,0xA200},{0x2DDC,0x9600},{0x3199,0x9B00},{0x3322,0x9600},
		{0x3367,0x9F00},{0x3372,0x9E00},{0x33F9,0x9E00},{0x3430,0x9C00},{0x3436,0x9700},
		{0x3442,0x9A00},{0x3449,0x9B00},{0x348A,0x9800},{0x3491,0x9900},{0x3498,0x9E00},
		{0x349D,0x9F00},{0x34C8,0x9600},{0x34F9,0x9F00},{0x34FE,0x9F00},{0x3503,0x9E00},
		{0x3508,0x9E00},{0x3525,0x9700},{0x352F,0x9600},{0x35C3,0x9900},{0x35CF,0x9B00},
		{0x3606,0x9B00},{0x360C,0x9600},{0x3615,0xA600},{0x3C4A,0x9600},{0x3CC0,0x9B00},
		{0x3CC9,0x9600},{0x3D4F,0x9B00},{0x6529,0x9C00},{0x652E,0x9C00},
	};
	for (int i = 0; i < 154; i++) {
	...
	// Bank-0 sites ($8000-$BFFF, in SROM paged bank) — use BBC.write so the
	// ROMSEL-aware Bus routes to sidewaysBanks[activeBank].
	static const uint16_t kBankObjSites[38][2] = {
		{0x9AD8,0x9A00},{0x9ADB,0x9800},{0x9B11,0x9C00},{0x9B20,0x9800},{0x9B6F,0xA300},
		{0x9B77,0xA700},{0x9B7C,0xA100},{0x9B87,0x9800},{0x9B8B,0x9A00},{0x9C66,0x9600},
		{0x9C83,0x9A00},{0x9CD4,0x9A00},{0x9CD9,0x9800},{0x9CE1,0x990F},{0x9D03,0x9C00},
		{0x9D0F,0x9800},{0x9D1B,0x9A00},{0x9D20,0xA600},{0x9E6C,0x9600},{0xA066,0x9600},
		{0xA08A,0x9600},{0xA337,0x9600},{0xA438,0x9600},{0xA462,0x9600},{0xA487,0x9800},
		{0xA48C,0x9A00},{0xA491,0x9F00},{0xA6AC,0x9600},{0xA6EC,0x9700},{0xA734,0x9600},
		{0xA8A5,0xA000},{0xA8B0,0xA700},{0xA8B6,0x9D00},{0xA8BB,0x9D00},{0xA8DB,0x9E00},
		{0xAA3B,0x9C00},{0xAA45,0xA100},{0xAA4B,0xA300},
	};
	for (int i = 0; i < 38; i++) {
